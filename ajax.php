...
// Assuming other actions are already defined above this

if (isset($_GET['action']) && $_GET['action'] == 'save_feedback') {
    session_start();
    include './db_connect.php';
    ...
    $stmt = $conn->prepare("INSERT INTO feedback_list (faculty_id, student_id, feedback) VALUES (?, ?, ?)");
    $stmt->bind_param("iis", $faculty_id, $student_id, $feedback);
...
success: function(resp) {
    ...
    if (resp == 1) {
        ...
        alert_toast("Error: " + resp, 'error'); // Display the specific error
    }
}
